...
It is worth noting where that purity comes from. The material entering the crucible was not filtered or re-melted into purity: it is the product of the **trichlorosilane distillation** described in [Purification & polysilicon](/en/polissilicio) <Cite id="wikipedia-tcs" />. It arrives already at **9N to 11N**, and what the pulling does from there is **redistribute** the few dopants that remain, according to each one's segregation coefficient <Cite id="saimm" />.
